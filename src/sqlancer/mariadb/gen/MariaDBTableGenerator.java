...
public class MariaDBTableGenerator {
    ...
    public static SQLQueryAdapter generate(MariaDBGlobalState globalState, String tableName) {
        return new MariaDBTableGenerator(globalState, tableName).create();
    }
    ...
    private SQLQueryAdapter create() {
        ...
        if (Randomly.getBoolean() && !schema.getDatabaseTables().isEmpty()) {
            ...
            if ((tableHasNullableColumn || setPrimaryKey) && engine == MariaDBEngine.CSV) {
                if (true) { // TODO
                    // results in an error
                    throw new IgnoreMeException();
                }
            } else if ((tableHasNullableColumn || keysSpecified > 1) && engine == MariaDBEngine.ARCHIVE) {
    ...
    private void appendPartitionOptions() {
        if (engine != MariaDBEngine.INNO_DB) {
            return;
        }
        if (Randomly.getBoolean()) {
            return;
        ...
        case HASH:
            ...
            // MariaDBExpression expr =
            // MariaDBRandomExpressionGenerator.generateRandomExpression(Collections.emptyList(),
            // null, r);
            // sb.append(MariaDBVisitor.asString(expr));
    ...
    private void appendTableOptions() {
        ...
        for (TableOptions o : tableOptions) {
            ...
            case ENGINE:
                ...
                this.engine = MariaDBEngine.get(fromOptions);
                sb.append("ENGINE = ");
    ...
    private enum ColumnOptions {
        NULL_OR_NOT_NULL, UNIQUE, COMMENT, PRIMARY_KEY //, COLUMN_FORMAT , STORAGE // TODO: see how STORAGE works; cannot find docs
    }

    private void appendColumnDefinition() {
        sb.append(" ");
        MariaDBDataType randomType = MariaDBDataType.getRandom(globalState);
        boolean isTextType = randomType == MariaDBDataType.VARCHAR;
    ...
    private void appendTypeString(MariaDBDataType randomType) {
        switch (randomType) {
        case DECIMAL:
            sb.append("DECIMAL");
        ...
        if (randomType.isNumeric()) {
            if (Randomly.getBoolean() && randomType != MariaDBDataType.INT && !MariaDBBugs.bug99127) {
                sb.append(" UNSIGNED");
            }
    ...
    public static void optionallyAddPrecisionAndScale(StringBuilder sb) {
        if (Randomly.getBoolean() && !MariaDBBugs.bug99183) {
            sb.append("(");
