...
public class MariaDBAlterTable {
    ...
    public MariaDBAlterTable(MariaDBSchema newSchema) {
        this.schema = newSchema;
    }

    public static SQLQueryAdapter create(MariaDBGlobalState globalState) {
        return new MariaDBAlterTable(globalState.getSchema()).create();
    }

    private enum Action {
        ALGORITHM, //
        CHECKSUM, //
        // COMPRESSION, // TODO: Compression not available on mariadb?
        DISABLE_ENABLE_KEYS("Data truncated for functional index"), /* ignore due to http://bugs.mariadb.com/?id=96295 */
        DROP_COLUMN("Cannot drop column", "ALGORITHM=INPLACE is not supported.", "ALGORITHM=INSTANT is not supported.",
                ...
                "A primary key index cannot be invisible" /*
                                                           ...
                                                           * https://bugs.mariadb.com/bug.php?id=95897
                                                           */,
    ...
    private SQLQueryAdapter create() {
        ExpectedErrors errors = ExpectedErrors.from("does not support the create option", "doesn't have this option",
                "is not supported", "Data truncation", "Specified key was too long");
        ...
        if (!table.hasPrimaryKey() || MariaDBBugs.bug95894) {
